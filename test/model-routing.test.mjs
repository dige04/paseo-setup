...
const inventory = {
	providers: [
		{ id: "claude-supervisor", enabled: true },
		{ id: "claude-lead", enabled: true },
		{ id: "claude-peer", enabled: true },
	],
	models: [
		{
...
}
{
	...
	badProvider.routes.FAST_READ.paseoProvider = "claude-peer-fast";
	expectRoutingError("CONFIG_INVALID", () =>
		validateRoutingConfig(badProvider),
	);
}
{
	// A bare model id is VALID here. Pi wrapped upstream providers so its models
	// were always "<pi-provider>/<model-id>"; Claude Code ids carry no prefix.
	const bareModel = structuredClone(validConfigData);
	bareModel.routes.FAST_READ.model = "claude-opus-5";
	validateRoutingConfig(bareModel);

	// A slash-bearing value is still split strictly: an empty segment on either
	// side is reported where the config author made the mistake.
	for (const malformed of ["/claude-opus-5", "vendor/", "claude opus 5"]) {
		const bad = structuredClone(validConfigData);
		bad.routes.FAST_READ.model = malformed;
		expectRoutingError("CONFIG_INVALID", () => validateRoutingConfig(bad));
	}
}
{
...
assert.deepEqual(splitProviderModel("claude-peer/testprov/coder-mid"), {
	provider: "claude-peer",
	model: "testprov/coder-mid",
});
// Multi-slash model IDs: split at FIRST slash only (mirrors Paseo source).
assert.deepEqual(splitProviderModel("claude-peer/vendor/scoped/deep-reasoner"), {
	provider: "claude-peer",
	model: "vendor/scoped/deep-reasoner",
});
assert.deepEqual(splitProviderModel("claude-peer/a/b/c/d"), {
	provider: "claude-peer",
	model: "a/b/c/d",
});
expectRoutingError("CONFIG_INVALID", () => splitProviderModel("claude-peer"));
expectRoutingError("CONFIG_INVALID", () => splitProviderModel("/model"));
expectRoutingError("CONFIG_INVALID", () => splitProviderModel("provider/"));

assert.equal(
	composeProviderModel("claude-peer", "vendor/scoped/deep-reasoner"),
	"claude-peer/vendor/scoped/deep-reasoner",
);
assert.equal(
	composeProviderModel("claude-supervisor", "testprov/fast-small"),
	"claude-supervisor/testprov/fast-small",
);
expectRoutingError("CONFIG_INVALID", () =>
	composeProviderModel("claude-peer-x", "a/b"),
);
// A bare model id is the normal Claude form ("claude-peer" + "claude-opus-5").
assert.equal(
	composeProviderModel("claude-peer", "claude-opus-5"),
	"claude-peer/claude-opus-5",
);
expectRoutingError("CONFIG_INVALID", () =>
	composeProviderModel("claude-peer", "   "),
);
expectRoutingError("CONFIG_INVALID", () =>
	composeProviderModel("claude-peer", "testprov/"),
);
expectRoutingError("CONFIG_INVALID", () =>
	composeProviderModel("claude-peer", "/m"),
);
expectRoutingError("CONFIG_INVALID", () => composeProviderModel("claude-peer", ""));
...

{
	...
	assert.equal(route.paseoProvider, "claude-peer");
	assert.equal(route.model, "testprov/coder-mid");
	assert.equal(route.thinking, "medium");
	assert.equal(route.createAgentProvider, "claude-peer/testprov/coder-mid");
...
{
	// Supervisor route goes through claude-supervisor profile.
	const route = resolveRoute(config, "MONITOR_ECONOMY", inventory);
	assert.equal(route.paseoProvider, "claude-supervisor");
	assert.equal(route.createAgentProvider, "claude-supervisor/testprov/fast-small");
}
...
expectRoutingError("ROLE_PROVIDER_UNAVAILABLE", () =>
	resolveRoute(config, "MONITOR_ECONOMY", {
		...
		providers: [
			{ id: "claude-peer", enabled: true },
			{ id: "claude-supervisor", enabled: false },
		],
	}),
);
expectRoutingError("ROLE_PROVIDER_UNAVAILABLE", () =>
...
// CLI --json shapes work too (provider/status/enabled strings, thinkingOptionIds).
{
	...
	// Fast-read needs claude-peer + testprov/fast-small (absent here) → MODEL_UNAVAILABLE.
	expectRoutingError("MODEL_UNAVAILABLE", () =>
...
expectRoutingError("ROLE_PROVIDER_UNAVAILABLE", () =>
	resolveRoute(config, "MONITOR_ECONOMY", {
		...inventory,
		providers: [{ id: "claude-supervisor", enabled: true, status: "error" }],
	}),
);
// Healthy statuses still pass.
{
	const okInventory = {
		...inventory,
		providers: [{ id: "claude-supervisor", enabled: true, status: "available" }],
	};
...

{
	...
	const raw = [
		{ provider: "claude-lead", status: "available", enabled: "Enabled" },
		{ provider: "claude-peer", status: "error", enabled: true },
		{ id: "claude-supervisor", enabled: true }, // MCP shape: no status field
		null,
		{ enabled: true }, // no id → dropped
	];
	const inv = buildProviderInventory(raw);
	assert.deepEqual(inv, [
		{ id: "claude-lead", enabled: true, status: "available" },
		{ id: "claude-peer", enabled: true, status: "error" },
		{ id: "claude-supervisor", enabled: true },
	]);
...
// (fail-closed; a lone `{ id: "claude-peer" }` must not be routed to).
for (const bad of [undefined, null, {}, "maybe", "disabled"]) {
	expectRoutingError("ROLE_PROVIDER_UNAVAILABLE", () =>
		resolveRoute(config, "CODING_MEDIUM", {
			...inventory,
			providers: [{ id: "claude-peer", enabled: bad }],
		}),
	);
}
...
assert.deepEqual(ROLE_PROVIDERS, ["claude-supervisor", "claude-lead", "claude-peer"]);
...
}
{
	...
	badRoute.hosts["mac-review"].routes.FAST_READ.model = "vendor/";
...
// resolveClusterRoute: exact host + class resolution against host inventory.
{
	...
	assert.equal(route.createAgentProvider, "claude-peer/testprov/reviewer-pro");
...
assert.notEqual(
	modelsCacheKey("mac-review", "claude-peer"),
	modelsCacheKey("linux-runner", "claude-peer"),
	"same role provider on two remote hosts must NOT share a cache entry",
);
assert.notEqual(
	modelsCacheKey("mac-review", "claude-peer"),
	modelsCacheKey("mac-review", "claude-lead"),
