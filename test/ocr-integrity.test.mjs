...
assert.match(installPs1, /claudeScriptsDir/);
...
assert.match(teamScriptsPath, /CLAUDE_TEAM_DIR/);
...
// Layer-1 local guard: the shared policy core owns the create_workspace args
// rule; every runtime binding must wire it in.
const policyCore = read("extensions/policy-core.mts");
assert.match(policyCore, /leadCreateWorkspaceArgsBlockReason/);
assert.match(policyCore, /REVIEW_WORKTREE_UNAVAILABLE/);
const claudeBinding = read("extensions/claude-policy.mts");
assert.match(claudeBinding, /leadCreateWorkspaceArgsBlockReason/);
...
assert.match(leadSkill, /PASEO_TEAM_SCRIPTS_DIR/);
