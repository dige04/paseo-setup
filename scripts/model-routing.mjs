...
export function defaultRoutingDir() {
	return process.env.PASEO_TEAM_HOME ?? join(homedir(), ".paseo-claude-team");
}
...
export function validateRoutingConfig(data) {
	...
	for (const [modelClass, route] of Object.entries(routes)) {
		...
		// A slash is optional here. Pi wrapped upstream providers, so its models
		// were always "<pi-provider>/<model-id>"; Claude Code model ids are bare
		// ("claude-opus-5"). Requiring a slash rejected every valid Claude route.
		// When a slash IS present, split the value DIRECTLY (not prefixed by
		// paseoProvider) so an empty segment is reported where the config author
		// made the mistake, not later at route composition.
		if (trimmedModel.includes("/")) splitProviderModel(trimmedModel);
		else if (/\s/.test(trimmedModel)) {
			throw fail(
				`route ${modelClass}: model "${trimmedModel}" must not contain whitespace`,
 ...
 * "claude-peer" + "claude-sonnet-5" → "claude-peer/claude-sonnet-5"
 * A model id may itself contain slashes; only the empty-segment forms are wrong.
...
export function composeProviderModel(paseoProvider, model) {
	...
	// A bare model id ("claude-sonnet-5") is the normal Claude form and composes
	// straight onto the role provider. Only validate the split when the value
	// actually carries one, so "<x>/" and "/<x>" are still rejected.
	if (trimmed.includes("/")) splitProviderModel(trimmed);
	else if (trimmed === "") {
		throw new RoutingError("CONFIG_INVALID", "model must be a non-empty string", { model });
	}
	if (trimmed.endsWith("/")) {
		throw new RoutingError(
			"CONFIG_INVALID",
