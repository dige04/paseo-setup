...
echo "     Measured 2026-09-01: provider-snapshot-manager writes the whole provider set"
echo "     back to config.json (on restart, and whenever the desktop app reconnects),"
echo "     materialising built-ins as enabled. An edit the daemon never loaded is"
echo "     silently clobbered, and even a loaded one reverts at the next snapshot"
echo "     refresh. Verify, and re-verify later:"
echo "       paseo provider ls | grep -E '^claude '   # want: unavailable  Disabled"
echo "     Treat the disabled base as a preference, not a guarantee. 'restart' kills"
echo "     running agents; use it only after a successful reload."
